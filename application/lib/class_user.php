<?php
  ...
  class Users
  {
      ...
      private function startSession()
      {
		...
		if (!$this->logged_in) {
			$this->username = $_SESSION['CMSPRO_username'] = "Guest";
	  ...
	  {
          if (isset($_SESSION['CMSPRO_username']) && $_SESSION['CMSPRO_username'] != "Guest") {
	  ...
	  public function login($username, $password)
	  {
		  ...
		  if (empty(Filter::$msgs) && $status == 5) {
			  ...
			  $this->username = $_SESSION['CMSPRO_username'] = $row->username;
			  $this->name = $_SESSION['CMSPRO_name'] = $row->fname.' '.$row->lname;
	  ...
	  public function getUserMembership()
	  {
          ...
          $sql = "SELECT u.*, m.title". Lang::$lang
		  . "\n FROM " . self::uTable. " as u"
		  . "\n LEFT JOIN " . Membership::mTable . " as m ON m.id = u.membership_id" 
		  . "\n WHERE u.id = " . $this->uid;
          $row = self::$db->first($sql);
