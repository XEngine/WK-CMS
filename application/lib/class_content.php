<?php
  ...
  class Content
  {
	  ...
	  public function getPages($paginate = false)
	  {
		  ...
		  $where = (Registry::get("Users")->userlevel == 8) ? "WHERE is_admin = 1" : null;
		  
		  $sql = "SELECT * FROM " . self::pTable
		  . "\n $where ORDER BY title" . Lang::$lang . $limit;
	  ...
	  public function getPortfolioSitemap()
	  {
		  ...
		  $sql = "SELECT title" . Lang::$lang . " as ptitle, slug FROM mod_portfolio ORDER BY title" . Lang::$lang;
		  $row = self::$db->fetch_all($sql);
	  ...
	  public function processPage()
	  {
		  ...
		  if (empty(Filter::$msgs)) {
			  ...
			  if ($data['contact_form'] == 1) {
				  $contact_form['contact_form'] = "DEFAULT(contact_form)";
				  self::$db->update(self::pTable, $contact_form);
			  }

			  if ($data['home_page'] == 1) {
				  $home_page['home_page'] = "DEFAULT(home_page)";
				  self::$db->update(self::pTable, $home_page);
			  }
	  ...
	  public function getPagePlugins()
	  {
		  ...
		  . "\n ORDER BY hasconfig DESC, title" . Lang::$lang . $pager->limit;
		  $row = self::$db->fetch_all($sql);
	  ...
	  public function getLayoutOptions($pageid, $modid = false)
	  {
		  ...
		  $sql = "SELECT l.*, p.id as plid, p.title" . Lang::$lang 
	  ...
	  public function getMenu($array, $parent_id = 0, $menuid = 'main-menu', $class = 'top-menu')
	  {
		  ...
		  if(is_array($array) && count($array) > 0) {
			  ...
			  $submenu = false;
			  $attr = (!$parent_id) ? ' class="' . $class . '" id="' . $menuid . '"' : ' class="sub"';
			  $attr2 = (!$parent_id) ? ' class="parent"' : ' class="parent"';
			  ...
			  foreach ($array as $key => $row) {
				  if ($row['parent_id'] == $parent_id) {
					  ...
					  $active = ($row['pslug'] == $this->slug) ? "active" : "normal";
					  $mactive = ($row['pslug'] == $this->modalias) ? "active" : "normal";
					  
					  $name = ($parent_id == 0)  ? $row['name' . Lang::$lang] : $row['name' . Lang::$lang];
					  
					  $homeactive = (preg_match('/index.php/', $_SERVER['PHP_SELF'])) ? "active" : "normal";
					  ...
					  switch ($row['content_type']) {
						  case 'module':
							  ...
							  $link = '<a href="' . $murl2 . '" class="' . $mactive . '">' . $name . '</a>';
							  break;
							  
						  case 'page':
							  if ($row['home_page']) {
								  $link = '<a href="' . SITEURL . '/" class="' . $homeactive . $home . '">' . $name . '</a>';
							  } else {
								  $link = '<a href="' . $url . '" class="' . $active . $home . '">' .  $name.'</a>';
							  }
							  break;
						  ...
						  case 'web':
							  ...
							  $link = '<a href="' . $wlink . '"' . $wtarget . '>' .  $name .'</a>';
							  break;
					  }
	  ...
	  public function addLanguage()
	  {
		  ...
		  if (empty(Filter::$msgs)) {
			  ...
			  self::$db->query("ALTER TABLE " . self::eTable . " ADD COLUMN name_$flag_id VARCHAR(200) NOT NULL AFTER name_en");
			  self::$db->query("ALTER TABLE " . self::eTable . " ADD COLUMN subject_$flag_id VARCHAR(255) NOT NULL AFTER subject_en");
			  self::$db->query("ALTER TABLE " . self::eTable . " ADD COLUMN help_$flag_id TEXT AFTER help_en");
			  self::$db->query("ALTER TABLE " . self::eTable . " ADD COLUMN body_$flag_id TEXT AFTER body_en");
			  ...
			  self::$db->query("ALTER TABLE " . Membership::mTable . " ADD COLUMN title_$flag_id VARCHAR(255) NOT NULL AFTER title_en");
			  self::$db->query("ALTER TABLE " . Membership::mTable . " ADD COLUMN description_$flag_id TEXT AFTER description_en");
			  self::$db->query("UNLOCK TABLES");
			  ...
			  self::$db->query("ALTER TABLE " . self::mTable . " ADD COLUMN name_$flag_id VARCHAR(100) NOT NULL AFTER name_en");
			  self::$db->query("ALTER TABLE " . self::mTable . " ADD COLUMN caption_$flag_id VARCHAR(100) NOT NULL AFTER caption_en");
			  self::$db->query("UNLOCK TABLES");
			  ...
			  self::$db->query("ALTER TABLE " . self::mdTable . " ADD COLUMN title_$flag_id VARCHAR(120) NOT NULL AFTER title_en");
			  self::$db->query("ALTER TABLE " . self::mdTable . " ADD COLUMN info_$flag_id TEXT AFTER info_en");
			  self::$db->query("ALTER TABLE " . self::mdTable . " ADD COLUMN metakey_$flag_id VARCHAR(200) NOT NULL AFTER metakey_en");
			  self::$db->query("ALTER TABLE " . self::mdTable . " ADD COLUMN metadesc_$flag_id TEXT AFTER metadesc_en");
			  self::$db->query("UNLOCK TABLES");
			  ...
			  self::$db->query("ALTER TABLE " . self::plTable . " ADD COLUMN title_$flag_id VARCHAR(120) NOT NULL AFTER title_en");
			  self::$db->query("ALTER TABLE " . self::plTable . " ADD COLUMN body_$flag_id TEXT AFTER body_en");
			  self::$db->query("ALTER TABLE " . self::plTable . " ADD COLUMN info_$flag_id TEXT AFTER info_en");
			  self::$db->query("UNLOCK TABLES");
			  ...
			  self::$db->query("ALTER TABLE " . self::pTable . " ADD COLUMN title_$flag_id VARCHAR(200) NOT NULL AFTER title_en");
			  self::$db->query("ALTER TABLE " . self::pTable . " ADD COLUMN caption_$flag_id VARCHAR(200) NOT NULL AFTER caption_en");
			  self::$db->query("ALTER TABLE " . self::pTable . " ADD COLUMN keywords_$flag_id TEXT AFTER keywords_en");
			  self::$db->query("ALTER TABLE " . self::pTable . " ADD COLUMN body_$flag_id TEXT AFTER body_en");
			  self::$db->query("ALTER TABLE " . self::pTable . " ADD COLUMN description_$flag_id TEXT AFTER description_en");
			  self::$db->query("UNLOCK TABLES");
			  ...
			  self::$db->query("ALTER TABLE " . self::cfTable . " ADD COLUMN title_$flag_id VARCHAR(100) NOT NULL AFTER title_en");
			  self::$db->query("ALTER TABLE " . self::cfTable . " ADD COLUMN tooltip_$flag_id VARCHAR(100) NOT NULL AFTER tooltip_en");
			  self::$db->query("UNLOCK TABLES");
	  ...
	  private function createSiteMap()
	  {
		  ...
		  if(isset($_POST['pf'])) {
		  $portfolio = self::$db->query("SELECT slug FROM mod_portfolio ORDER BY created DESC");
			  
			while ($row = self::$db->fetch($portfolio)) {
				$url = Url::Portfolio("item", $row->slug);
	  ...
	  public function getMeta()
	  {
		  ...
		  $meta .= "<meta name=\"generator\" content=\"Powered by Webkokteyli Labs! v1.0\" />\n";
		  return $meta;
	  }
  }
?>