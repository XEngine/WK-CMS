<?php
  ...
  require_once(THEMEPATH . "wk-yonet/plugins/slider/admin_class.php");
...
<section id="jqslider">
  <div class="ns-slider">
    <?php foreach ($slides as $srow):?>
      ...
      <?php else:?>
      <a href="<?php echo ($srow->urltype == "ext") ? $srow->url : SITEURL . "/" . $srow->url;?>" <?php echo ($srow->urltype == "ext") ? "target=\"_blank\"" : "target=\"_self\"";?>><img src="<?php echo SITEURL . '/application/' . Slider::imgPath . $srow->thumb;?>" <?php if($conf->showCaptions):?>data-slidecaption="<h1><?php echo $srow->{'title'.Lang::$lang};?><h1><?php echo $srow->{'body'.Lang::$lang};?>"<?php endif;?> alt=""></a>
      <?php endif;?>
    <?php endforeach;?>
    <?php unset($srow);?>
  </div>
