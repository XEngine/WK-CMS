<?php
  ...
  $keyword = sanitize($keyword, 15, false);
  $keyword = $db->escape($keyword);
?>
<?php $latestrow = Registry::get("Blog")->searchArticle($keyword);?>
<?php if (!$keyword || strlen($keyword = trim($keyword)) == 0 || strlen($keyword) < 3):?>
<?php Filter::msgAlert(Lang::$word->_SR_SEARCH_EMPTY2);?>
<?php elseif(!$latestrow):?>
<?php Filter::msgAlert(Lang::$word->_SR_SEARCH_EMPTY . '<span class="wk black label">'.$keyword.'</span>' . Lang::$word->_SR_SEARCH_EMPTY1);?>
<?php else:?>
<h1 class="wk double header"><span><?php echo '<small class="wk label">' . count($latestrow) . '</small> ' .Lang::$word->_SR_SEARCH2 . $keyword;?></span></h1>
  ...
  <article class="clearfix">
    ...
    <section>
      ...
      <figure class="small-top-space small-bottom-space">
        <div class="image-overlay wk left floated image"> <img src="<?php echo SITEURL;?>/thumbmaker.php?src=<?php echo SITEURL.'/'.Blog::imagepath . $row->thumb;?>&amp;h=150&amp;w=250&amp;s=1&amp;a=tl" alt="">
