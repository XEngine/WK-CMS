...
<div class="wk horizontal divided list">
...
<article id="article" class="clearfix">
  <figure class="small-top-space small-bottom-space">
    ...
    <?php else:?>
    <?php if($row->thumb):?>
    <div class="image-overlay wk right floated image"> <img src="<?php echo SITEURL;?>/thumbmaker.php?src=<?php echo SITEURL.'/'.Blog::imagepath . $row->thumb;?>&amp;h=200&amp;w=300&amp;s=1&amp;a=tl" alt="">
  ...
  <a href="<?php echo Url::Blog("item", $relrow->slug);?>" class="wk label"><?php echo $relrow->atitle;?></a>
  <?php endforeach;?>
  <div class="wk divider"></div>
  <?php endif;?>
  <?php if($row->show_sharing or $row->show_like):?>
  <div class="content"> <a target="_blank" data-content="<?php echo Lang::$word->_MOD_AM_SHARE;?> Facebook" href="https://www.facebook.com/sharer/sharer.php?u=<?php echo Url::Blog("item", $row->slug);?>" class="wk icon facebook button"><i class="icon facebook"></i></a> <a data-content="<?php echo Lang::$word->_MOD_AM_SHARE;?> Twitter" href="https://twitter.com/home?status=<?php echo Url::Blog("item", $row->slug);?>" class="wk icon twitter button"><i class="icon twitter"></i></a> <a target="_blank" data-content="<?php echo Lang::$word->_MOD_AM_SHARE;?> Google +" href="https://plus.google.com/share?url=<?php echo Url::Blog("item", $row->slug);?>" class="wk icon google plus button"><i class="icon google plus"></i></a> <a target="_blank" data-content="<?php echo Lang::$word->_MOD_AM_SHARE;?> Pinterest" href="https://pinterest.com/pin/create/button/?url=&amp;media=<?php echo Url::Blog("item", $row->slug);?>" class="wk icon pinterest button"><i class="icon pinterest"></i></a>
    <?php if($row->show_like):?>
    <a data-id="<?php echo $row->id;?>" data-total="<?php echo $row->like_up;?>" class="wk like danger labeled icon button push-right"> <i data-content="<?php echo Lang::$word->_MOD_AM_LIKE;?>" class="heart icon"></i> <span><?php echo $row->like_up;?></span> </a>
    <?php endif;?>
  </div>
  <div class="wk divider"></div>
  <?php endif;?>
  ...
  <a href="<?php echo Url::Blog("blog-tags", $tag);?>" class="wk tag label"><i class="icon tag"></i> <?php echo $tag;?></a>
  <?php endforeach;?>
  <div class="wk divider"></div>
  <?php endif;?>
</article>
