...
<?php
  ...
  if (isset($_POST['userSearch'])):
      ...
      if (strlen($string) > 3):
          ...
          if ($result = $db->fetch_all($sql)):
              $html .= '<div id="search-results" class="wk segment celled list">';
              foreach ($result as $row):
                  $thumb = ($row->avatar) ? '<img src="' . UPLOADURL . 'avatars/' . $row->avatar . '" alt="" class="wk image avatar"/>' : '<img src="' . UPLOADURL . 'avatars/blank.png" alt="" class="wk image avatar"/>';
  ...
  if (isset($_POST['loadLanguage'])):
      if (file_exists(BASEPATH . Lang::langdir . Core::$language . "/" . $_POST['filename'] . ".xml")):
          ...
          foreach ($xmlel as $pkey):
              $html .= '<div class="row">';
              $html .= '<div contenteditable="true" data-path="' . $_POST['filename'] . '" data-edit-type="language" data-id="' . $i++ . '" data-key="' . $pkey['data'] . '" class="wk phrase">' . $pkey . '</div>';
              $html .= '</div>';
          endforeach;
          $json['status'] = 'success';
          $json['message'] = $html;
  ...
  if (isset($_POST['loadAvailPlugs'])):
      ...
      if (!$pluginrow):
          ...
          print '<div class="wk-grid">';
          print '<div class="three columns small-horizontal-gutters wk sortable list">';
...
<?php
  ...
  if (isset($_POST['newNote'])):
      if (!empty($_POST['note'])):
          ...
          if ($lastid):
              $html = '
			  <div class="wk note ' . sanitize($_POST['color']) . '">
