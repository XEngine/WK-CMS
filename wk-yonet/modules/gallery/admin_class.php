<?php
  ...
  class Gallery
  {
	  ...
	  public function processGallery()
	  {
		  ...
		  if (!Filter::$id) {
			  Filter::checkPost('folder', Lang::$word->_MOD_GA_FOLDER);
		  }
		  ...
		  if (empty(Filter::$msgs)) {
			  ...
			  if (!Filter::$id) {
				  $data['folder'] = doSeo($_POST['folder']);
				  $data['created'] = "NOW()";
			  }
	  ...
	  public static function doUpload($filename)
	  {
		  ...
		  if (isset($_FILES[$filename]) && $_FILES[$filename]['error'] == 0) {
			  ...
			  if (move_uploaded_file($_FILES[$filename]['tmp_name'], $fullname)) {
				  ...
				  $html = '
					  <div class="item"><a data-id="' . $last_id . '" data-name="-/-" class="imgdelete wk top right negative corner label"><i class="icon remove sign"></i></a>
						<a href="' . $url . '" class="lightbox" title="-/-"> <img src="' . $url . '" alt="" class="wk image"></a>
						<div class="wk-content">
						  <div contenteditable="true" placeholder="' . Lang::$word->_MOD_GA_IMG_TITLE . '" data-path="false" data-edit-type="gallery" data-id="' . $last_id . '" data-key="title" class="wk editable"></div>
						  <div class="wk small divider"></div>
						  <div contenteditable="true" placeholder="' . Lang::$word->_MOD_GA_IMG_DESC . '" data-path="false" data-edit-type="gallery" data-id="' . $last_id . '" data-key="desc" class="wk editable"></div>
						</div>
					  </div>';
