<?php
  ...
  class Blog
  {
	  ...
	  {
		  if (isset($_COOKIE['RATE_BLG_CMSPRO_'])) {
			  if($_COOKIE['RATE_BLG_CMSPRO_'] == $id) {
	  ...
	  {
		  $data = "<span class=\"wk rating-read\">";
	  ...
	  {
			$data = "<span class=\"wk rating-vote $class\">";
			$rating = ($ratingc == 0) ? 0 :  $ratingt/$ratingc;
			$count = 5;
			for ($i = 0; $i < 5; $i++) {
				$j = $i + 1;
	  ...
	  public function processArticle()
	  {
		  ...
		  Filter::checkPost('tags', Lang::$word->_MOD_AM_TAGS_R);
		  Filter::checkPost('short_desc' . Lang::$lang, Lang::$word->_MOD_AM_SHORTDESC);
	  ...
	  public static function listMemberships($memid)
	  {
		  ...
		  if ($data) {
			  ...
			  $display .= '<ul class="wk numbered list">';
			  foreach ($data as $row) {
