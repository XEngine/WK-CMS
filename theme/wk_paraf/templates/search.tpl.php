...
<?php Filter::msgAlert(Lang::$word->_SR_SEARCH_EMPTY . '<span class="wk black label">'.$keywords.'</span>' . Lang::$word->_SR_SEARCH_EMPTY1);?>
<?php else:?>
...
<?php foreach($contentdata as $cdata):?>
	  <?php
		  ...
		  if ($matches[1]) {
			...
			$newbody = preg_replace("|($keywords)|Ui", "<span class=\"wk warning label\">$1</span>", $string);
		  }
      ?>
<?php $i++;?>
